...
def generate_pdf_with_reportlab(text: str) -> bytes:
    """Строгий минималистичный PDF как на примере пользователя"""
    try:
        ...
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=50, rightMargin=50, topMargin=70, bottomMargin=40)
        story = []
        styles = getSampleStyleSheet()
        # Кастомные стили
        title_style = ParagraphStyle(
            'Title', fontName='LiberationSans-Bold', fontSize=32, textColor=HexColor('#23272f'), leftIndent=0, spaceAfter=8, alignment=0
        )
        date_style = ParagraphStyle(
            'Date', fontName='LiberationSans', fontSize=13, textColor=HexColor('#23272f'), leftIndent=0, spaceAfter=18, alignment=0
        )
        msg_style = ParagraphStyle(
            'Msg', fontName='LiberationSans', fontSize=18, textColor=HexColor('#23272f'), leftIndent=0, spaceAfter=0, alignment=0
        )
        footer_style = ParagraphStyle(
            'Footer', fontName='LiberationSans', fontSize=12, textColor=HexColor('#6b7280'), alignment=1, spaceBefore=10
        )
        # 1. Заголовок
        story.append(Paragraph("Chat Export", title_style))
        # 2. Горизонтальная синяя линия
        story.append(Spacer(1, 2))
        story.append(Table([[""]], colWidths=[160*mm], rowHeights=[2], style=[('BACKGROUND', (0,0), (-1,-1), HexColor('#1da1f2'))]))
        story.append(Spacer(1, 16))
        # 3. Дата
        date_str = f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        story.append(Paragraph(date_str, date_style))
        # 4. Сообщение в плашке с синей левой рамкой
        msg_box = Table(
            [[Paragraph(text, msg_style)]],
            colWidths=[160*mm],
            style=[
                ('BACKGROUND', (0,0), (-1,-1), HexColor('#f7f9fa')),
                ('LINEBEFORE', (0,0), (0,-1), 4, HexColor('#1da1f2')),
        ...
        # 5. Серая линия
        story.append(Table([[""]], colWidths=[160*mm], rowHeights=[1], style=[('BACKGROUND', (0,0), (-1,-1), HexColor('#e5e7eb'))]))
        story.append(Spacer(1, 8))
        # 6. Футер
        story.append(Paragraph("Generated by PPCSet AI Assistant<br/>https://www.ppcset.com", footer_style))
        doc.build(story)
