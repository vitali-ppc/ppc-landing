...
> ✅ **ЗАКРЫТО 2026-05-12** — задеплоено в продакшен на `https://www.kampaio.com` за ~3 часа.
> Этот документ оставлен как **исторический референс** + чек-лист на случай повторного деплоя на свежем сервере.
>
> Производственные данные → [`HANDOFF.md`](./HANDOFF.md) → секция «Production URLs».
> Технический deployment manual → [`DEPLOY.md`](./DEPLOY.md).

---

Цель (исходно): за **2 часа** получить рабочий `https://kampaio.com` в интернете с beta-доступом.

> Реальный launch занял ~3 часа из-за двух найденных багов в проде: missing `psycopg2-binary` в requirements.txt + Next.js 15.3.5 vulnerable (Vercel блокирует). Оба пофикшены в `181efe6` + `5bb3afe`.
